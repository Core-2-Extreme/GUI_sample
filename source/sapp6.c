...
//Defines.
//System UI.
#define DEF_SAPP6_HID_SYSTEM_UI_SEL(k)			(bool)((DEF_HID_PHY_PR(k.touch) && DEF_HID_INIT_IN((*Draw_get_bot_ui_button()), k)) || DEF_HID_PHY_PR(k.start))
#define DEF_SAPP6_HID_SYSTEM_UI_CFM(k)			(bool)(((DEF_HID_PR_EM(k.touch, 1) || DEF_HID_HD(k.touch)) && DEF_HID_INIT_LAST_IN((*Draw_get_bot_ui_button()), k)) || (DEF_HID_PR_EM(k.start, 1) || DEF_HID_HD(k.start)))
#define DEF_SAPP6_HID_SYSTEM_UI_DESEL(k)		(bool)(DEF_HID_PHY_NP(k.touch) && DEF_HID_PHY_NP(k.start))
...
//Variables.
static bool sapp6_main_run = false;
static bool sapp6_thread_run = false;
static bool sapp6_already_init = false;
static bool sapp6_thread_suspend = true;
static Thread sapp6_init_thread = NULL, sapp6_exit_thread = NULL, sapp6_worker_thread = NULL;
static Str_data sapp6_status = { 0, };
static Str_data sapp6_msg[DEF_SAPP6_NUM_OF_MSG] = { 0, };
...
void Sapp6_hid(Hid_info key)
{
	...
	else
	{
		...
		if(DEF_SAPP6_HID_SYSTEM_UI_SEL(key))
			Draw_get_bot_ui_button()->selected = true;

		//Execute functions if conditions are satisfied.
		if(DEF_SAPP6_HID_SYSTEM_UI_CFM(key))
			Sapp6_suspend();
	}

	//Notify user that button is NOT being pressed anymore.
	if(DEF_SAPP6_HID_SYSTEM_UI_DESEL(key))
